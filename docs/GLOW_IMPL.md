...
- **3c — host-obligation doc + the CI feature-unification guard** (`cargo tree -i wgpu` empty on `--no-default-features --features grab-pass`, asserted in the `grab-pass` CI job alongside build + clippy + the orientation tests). *Green:* `cargo build/clippy/test -p backdrop-blur-egui --no-default-features --features grab-pass`. **Test coverage as built:** the load-bearing orientation wiring (egui geometry → GL bottom-left region, no flip) is a **pure** `callback_region` unit test (4 cases: egui-top→GL-high-y, egui-bottom→GL-low-y, ppp scaling, disjoint→None) — runs on any runner, no GL. **Deferred (recorded, not silently skipped):** the automated *egui+GL* integration test (a real egui_glow frame frosted through the callback, the grab-pass analog of `own_loop_render.rs`) is **not** built — the EGL harness is crate-internal to `backdrop-blur-glow` (`#[cfg(test)] mod gl_harness`), so an egui-crate integration test would duplicate ~150 lines of EGL setup. Integration is instead proven by the **Step-5 eframe-glow example** (a real frame through `GrabPassRenderer`, manual/visual), and the glow backend's own 14 Tier-1 readback tests prove grab→blur→composite. A future shared `*-test-support` crate would let the automated egui-integration test (and the full-fb-viewport-override regression guard the orientation review flagged) land cleanly.
...
- **4a — native `gl-snapshots`.** The test-support EGL-surfaceless harness was **built in Step 2b** (`crates/backdrop-blur-glow/tests/gl_harness/`) so each Tier-1 test could run as the GL code landed; Step 4 only adds the **CI job** that runs it. As-built reality (the design's `EGL_PLATFORM_SURFACELESS_MESA` guess was wrong for this host): the harness uses the NVIDIA **`EGL_EXT_platform_device`** path (`eglQueryDevicesEXT` → `eglGetPlatformDisplayEXT`, raw FFI via `get_proc_address`+`transmute`), `eglBindAPI(OPENGL)`, a 3.3 context with **the profile mask omitted** (NVIDIA rejects it), surfaceless `make_current(NO_SURFACE)`, then `glow::Context::from_loader_function_cstr`. Yields **desktop GL 3.3** (`#version 140`); deps `khronos-egl` (dynamic) + `libloading` as **`[target.'cfg(not(target_arch = "wasm32"))'.dev-dependencies]`**. Compile-gated, must-pass on the GL CI runner, absent elsewhere. `--test-threads=1`.
