...
**Context-loss identity.** On web a `webglcontextlost`/`restored` cycle hands a *new* `glow::Context` while `GlowBlur` holds resources minted on the dead one. Resources are **stamped with a context identity**; on mismatch `GlowBlur` discards stale handles *without* GL deletes and rebuilds, and `frost` falls back to flat between lost and restored. (This composes with the no-GL-in-`Drop` rule above: stale handles are dropped, never deleted, against the dead context.)
