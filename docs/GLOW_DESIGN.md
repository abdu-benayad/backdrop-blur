...
**Callback-ran detection (corrected protocol, post-Abdu-review).** egui_glow **drops** a callback whose payload is not the *exact* `egui_glow::CallbackFn` type the consumer's eframe was compiled against, emitting an **uncatchable egui-side `log::warn!`** (painter.rs:442) — the host gets no `Result`. The protocol must respect that **`prepare` runs *inside* the callback** (it consumes the grabbed texture, which exists only there) — so `frost`, at layout time, cannot gate on `prepare`'s result. The working protocol: `frost`'s **enqueue gate** is what it *can* know up front — the surface's request resolves live *and* its rect is non-degenerate in egui coords; on enqueue it clears an `Arc<AtomicBool>`; the **callback sets the flag at entry, before `prepare`**; the adapter warns **once** next frame iff `{enqueued ∧ flag still false}`. A clipped-to-empty panel then cannot false-positive — the callback still *ran* (and set the flag) even though `prepare` returned `None`. The ran-flag is the *sufficient* programmatic guard; pinning `egui_glow` to the consumer's eframe minor is best-effort (a library cannot enforce the consumer's dep tree); two `egui_glow` copies in the tree is the residual mode the flag catches.
