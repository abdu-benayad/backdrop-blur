...
- **`decode_srgb = 1` always; `encode_srgb` follows the *captured target's* sRGB state (C2, refined post-Abdu-review).** egui always writes gamma/sRGB bytes (egui#3168), so the grab decode is unconditional. The encode bit is **detected at runtime from the real target** the callback captured (§5), not hard-wired: on native, `encode_srgb = !glIsEnabled(GL_FRAMEBUFFER_SRGB)` — for the default framebuffer this is `1` (GL defaults `GL_FRAMEBUFFER_SRGB` off and egui_glow never enables it on this path: painter.rs:326-327, 177), and for egui's `pp_fb_extent` post-process FBO (which *can* enable framebuffer sRGB, painter.rs:131) it is `0` so the hardware encodes — no double-encode; on web, WebGL2 has no `GL_FRAMEBUFFER_SRGB` and the default drawing buffer is non-encoding, so `encode_srgb = 1` always. This replaces v1's swapchain-`TextureFormat` allowlist (glow never introspects the `u32` internal format) **and turns the former `pp_fb_extent` caveat into handled behavior** — the encode decision rides the same captured target the grab/composite already follow (§5). A Tier-1/Tier-2 GLES assertion still backstops a surprising driver.
...
- **Kiosk GLES3 *correctness*.** `EXT_color_buffer_float` absence (→ RGBA8 scratch), 6-level pyramid fill-rate, and silent `highp→mediump` affect whether the *reference* reproduces on embedded GLES3. SwiftShader is a GLES3 proxy in Tier 2; real-device confirmation is deferred with the perf benchmark. (The former `pp_fb_extent` sRGB caveat is now **handled**, not a risk — encode follows the captured target's sRGB state, §5/§10.)
