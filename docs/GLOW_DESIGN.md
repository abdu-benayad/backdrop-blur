...
- **`GlowBlur`** — the cached, cross-frame GL resources: blur programs (Gaussian + Kawase down/up), the composite program, per-size scratch chains, the grab texture, the shared VAO. Implements `BackdropBlur` + `GrabPass`. Sole owner of GL object lifetimes; teardown is **explicit, never in `Drop`** (see below).
...
**Explicit teardown — `Drop` issues no GL (mandated, post-Abdu-review).** At app exit nothing guarantees the `glow::Context` is current — or alive — when `Drop` runs, so a `glDelete*` in `Drop` is a UB hole (native) the web identity-stamp alone does not close. The design follows `egui_glow::Painter::destroy`: `GlowBlur` (and `GrabPassRenderer`) expose **`destroy(&glow::Context)`**, which the consumer calls from eframe's glow-gated `on_exit(gl: Option<&glow::Context>)`. **`Drop` never issues a GL call** — if `destroy` was not called it `log_warn`s the leak. This deletes an entire UB class from the boundary rather than relying on Drop-time context liveness.

**Context-loss recovery — the host rebuilds; the backend stamps nothing.** On web a `webglcontextlost`/`restored` cycle hands a *new* `glow::Context` while a `GlowBlur` holds resources minted on the dead one. These are **canvas-level DOM events the backend cannot observe** (it has no canvas and no event loop), so detection — and therefore the response — belong to the host. Recovery composes directly with the no-GL-in-`Drop` rule above: on restore the host **drops the old `GrabPassRenderer` and constructs a new one** against the fresh context (exactly how `egui_glow` rebuilds its `Painter`). `GlowBlur::Drop` issues no GL, so the dead context's handles are *abandoned, not deleted* — and nothing actually leaks, the GPU objects died with the context. The backend therefore holds **no context identity and runs no per-call identity check**: a `GlowBlur` is only ever handed the context it was born from, which is a structural fact, not something to re-verify each frame (paying a runtime check to guard an invariant is the §5 smell — make it structural). Between lost and restored `frost` is flat — and until the web real-blur path ships (§9, Tier-2(B), deferred) web is flat regardless, so this is a *contract*, not yet-running code. *(That web increment owns the eframe-web restore hook this needs: `eframe::App` exposes `on_exit(gl)` but no `on_context_restored` today.)*

**The `Arc<Mutex<GlowBlur>>` is a bound-satisfier, not a concurrency model.** `egui_glow::CallbackFn` requires `Send + Sync + 'static` and `prepare` needs `&mut self`, so the shared handle is a `Mutex`. The seam is frame-serial single-threaded; the Mutex is never contended. It is documented as such; the callback carries a `debug_assert` it runs on the thread the renderer was constructed on — the fixed paint thread, per the documented eframe wiring (off-thread GL is UB). The poisoned-mutex path is handled (flat fallback + `log_err`), never silently skipped.
...
**Frost-outcome detection (widened from the ran-flag, F3).** egui_glow **drops** a callback whose payload is not the *exact* `egui_glow::CallbackFn` type the consumer's eframe was compiled against, emitting an **uncatchable egui-side `log::warn!`** (painter.rs:442) — the host gets no `Result`. The protocol must respect that **`prepare` runs *inside* the callback** (it consumes the grabbed texture, which exists only there) — so `frost`, at layout time, cannot gate on `prepare`'s result, and the report flows through a shared atomic instead: an `Arc<AtomicU8>` encoding a **`FrostOutcome`** (`DidNotFire = 0 < ClippedEmpty < Failed < Composited`, adapter vocabulary — it lives in `backdrop-blur-egui`, not core). Each callback **commits at its exit paths** — adapter-level clip to nothing → `ClippedEmpty`, backend `Err` → `Failed`, `Ok(FrostEffect)` → `Composited`/`ClippedEmpty` — via `fetch_max`, so a multi-surface frame ratchets to the strongest result (deliberately: the value answers "did the glass appear?"; a same-frame `Failed` is masked by a composite and stays on the throttled `log::warn!` channel). A **drop guard armed at entry** commits `Failed` if the callback unwinds first, so a fired-then-panicked callback never reads back as `DidNotFire` (unwind-only: `panic = "abort"` runs no destructor). The host polls **`take_frost_outcome()`** once per frame after paint — read-and-clear, "the strongest outcome since the last take"; `DidNotFire` for an enqueued frost is the version-skew tell. Pinning `egui_glow` to the consumer's eframe minor is best-effort (a library cannot enforce the consumer's dep tree); two `egui_glow` copies in the tree is the residual mode the outcome catches.

**Feature-gating — the exact, breaking edits (M5).** `backdrop-blur-egui` today depends on `wgpu`/`egui-wgpu`/`backdrop-blur-wgpu` *unconditionally* and re-exports wgpu/egui-wgpu types **and `strongest_repaint`** at the crate root with no `cfg`. The refactor: mark those three deps `optional = true`; define `own-loop` (default) and `grab-pass` feature→dep maps; gate `mod own_loop` and **all** of those re-exports behind `#[cfg(feature = "own-loop")]` (a semver-relevant public-API-shape change); both glow consumers — web *and* kiosk — build `default-features = false, features = ["grab-pass"]`. **One crate vs a split `backdrop-blur-egui-glow`:** kept one feature-gated crate (STRUCTURE intent + small hoisted spine), re-confirmed *after* pricing the cfg-gated re-export surface above — that cost, not aesthetics, decides it (§17). **Feature-unification guard (post-Abdu-review):** because `own-loop` is a *default* feature, any workspace member depending on `backdrop-blur-egui` with default features would silently re-unify wgpu into the kiosk build. CI builds both combos and a **kiosk-profile job asserts wgpu is absent from resolution** — `cargo tree -i wgpu` on the `--no-default-features --features grab-pass` build must come back empty. The escape hatch, if that guard ever trips in practice, is to make `own-loop` non-default (forcing explicit feature selection at the cost of breaking the v1 default ergonomic). The single-crate decision holds for a **closed workspace** (kiosk-as-consumer, which is the case); see §17 for the publish trigger.
...
- **Two surfaces, two formats — do not conflate.** The **grab** texture is **always RGBA8** on *all* targets (egui renders 8-bit gamma regardless of format — egui#3168 — so RGBA8 is the precision floor, not a fallback). The **blur scratch** is **RGBA16F** when `GlProfile`'s renderable-float tier is `Rgba16F`, and degrades to an **sRGB-encoded RGBA8 scratch** (`Srgb8Rgba8` tier — decode→tap→re-encode each pass, perceptually-uniform 8-bit, logged) only when float is not renderable: web without `EXT_color_buffer_float`, or a float-incapable kiosk. The float-tier enum keys the scratch format; the grab format is fixed. Correctness on the fallback rests on the **automatic** sRGB encode-on-write of its only targets (GLES 3.0 / WebGL2 — no `GL_FRAMEBUFFER_SRGB` to gate it, GLES 3.0.6 §4.1.8; the sample-side decode is unconditional everywhere). It is therefore **embedded-only by construction**: `GlProfile::classify` gives desktop GL `RGBA16F` (whose encode would otherwise need the default-off `FRAMEBUFFER_SRGB`), pinned by a `debug_assert`.
...
- **No automatic sRGB on the default framebuffer** — true on native too (§10): the default drawing buffer is non-sRGB, so the composite manually encodes there (an sRGB *user* target instead lets the hardware encode; §10).
...
- **`decode_srgb = 1` always; `encode_srgb` is resolved from the *captured target's colour attachment* (G1/G2, attachment-resolved).** egui always writes gamma/sRGB bytes (egui#3168), so the grab decode is unconditional. The encode bit is resolved once, at `record` time, from the real target the callback captured (§5): `resolve_target_encoding` reads the bound draw FBO's `GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING` and — **only where the enable is a valid capability** (desktop GL, or GLES advertising `EXT_sRGB_write_control`) — the `GL_FRAMEBUFFER_SRGB` write-encode state; the shader encodes iff the hardware will not. This is **correct by construction** on any (attachment × enable) pair, needing no assumption about egui_glow's enable handling: it **matches** the old `!glIsEnabled(GL_FRAMEBUFFER_SRGB)` on every well-behaved desktop path (a non-sRGB default framebuffer with the enable off → manual encode), and **fixes** the two the old global-enable proxy got wrong — a non-sRGB target with the enable left on (was washed out), and a core-GLES3/WebGL2 sRGB target (the old query raised `GL_INVALID_ENUM` every frame; the enable is now never queried where the enum does not exist, G1). This replaces v1's swapchain-`TextureFormat` allowlist (glow never introspects the `u32` internal format); glow now speaks the same `TargetEncoding` as wgpu, resolving it at a *different phase* — record, from the bound attachment — than wgpu (prepare, from the static `TextureFormat`), because the seam hands the framebuffer only to `record`. **No egui-driven path currently produces an sRGB-capable target**: the pinned `egui_glow 0.34.3` has no `pp_fb_extent` field (a vestigial doc comment only), `intermediate_fbo()` returns `None`, and `srgb_textures` is hardcoded `false` — so G2 is a *latent, host-facing* correctness property (a host binding its own sRGB render target before `frost_region`), not an egui bug today. The desktop user-FBO paths are guarded by a render+readback (§14, test 5b); the embedded default-FB query stays deferred to the web/GLES real-device tier.
...
- **All `unsafe` lives in `backdrop-blur-glow`.** core, wgpu, and the egui adapter stay `#![forbid(unsafe_code)]`. The glow crate carries `#![deny(unsafe_op_in_unsafe_fn)]` **and `#![deny(clippy::undocumented_unsafe_blocks)]`** (post-Abdu-review), so the **`// SAFETY:` discipline is clippy-enforced, not reviewer-memory** — every block gets one or the crate does not compile.
...
- **No GL in `Drop` — explicit teardown closes the native UB hole (post-Abdu-review).** `Drop` issues no GL call (at app exit the context may be non-current or gone); `GlowBlur`/`GrabPassRenderer` expose `destroy(&glow::Context)` for the consumer's glow-gated `on_exit(gl)`, and `Drop` `log_warn`s if `destroy` was skipped (§6). This removes Drop-time context-liveness from the unsafe surface entirely.
...
| Callback never ran (version skew) | `take_frost_outcome()` reports `DidNotFire` for an enqueued frost (§7) — the host's wiring check. |
| Web context lost (`webglcontextlost`) | Canvas DOM event the host observes: it drops + rebuilds `GrabPassRenderer` on restore. `Drop` abandons handles with no GL (the objects died with the context); flat until restored (§6). |
...
- **Linear light, resolved encode:** convolution is gamma-correct; the composite re-encodes to sRGB exactly once — manually (`encode_srgb = 1`) for a non-encoding target, or via the hardware for an sRGB target, per the encoding resolved from the captured attachment (§10).
...
- **Kiosk GLES3 *correctness*.** `EXT_color_buffer_float` absence (→ RGBA8 scratch), 6-level pyramid fill-rate, and silent `highp→mediump` affect whether the *reference* reproduces on embedded GLES3. SwiftShader is a GLES3 proxy in Tier 2; real-device confirmation is deferred with the perf benchmark. (A host post-process sRGB FBO is **handled**, not a risk — grab, composite, and the attachment-resolved encode all follow the captured target, §5/§10.)
...
- **Coordinate-convention divergence (§5).** The bottom-left flip moves out of `grab_source` to the callback (the `GrabPass` seam doc + glow-gate table are updated to match); the GL-origin space is now a **mandated type**, not a flagged smell.
- **One-crate-vs-split, with a publish trigger (§7).** Decided one feature-gated crate + a CI feature-unification guard (`cargo tree -i wgpu` empty on the grab-pass-only build), valid for the **closed kiosk workspace**. **If publishing this crate becomes plausibly near-term, fold the split into the M5 break** — foreign workspaces' default-feature unification is outside our control, and a second breaking change later is costlier than splitting inside the break already happening.
...
**Round 2 — focused correctness verification of the fixes (17 findings, all acted on):** the real catch was that a full-screen composite triangle under egui's **panel-sized viewport** loses straight-edge AA — fixed by overriding to a **full-framebuffer viewport** (§10/§11). The clean simplification: glow's GLSL composite is a **separate file** from wgpu's WGSL, so glow emits premultiplied with **no wgpu edit and no shared flag** (§10/§13), and the premultiplied output is **algebraically identical** to the frozen straight-alpha path under encode-then-coverage. Also: the whole `BlurRequest` is built bottom-left on the glow path (not just `rect_origin`); `clip_rect ∩` is a step distinct from `clip_to`; §8 desktop header is `#version 140` (mirroring egui_glow's pattern, not reusing its output, not `330 core`); the premultiplied no-halo assertion content pinned in §14; `strongest_repaint` is a public re-export (part of the M5 break); "silently no-ops" → an uncatchable `log::warn!`; the `material.rs` cite corrected.

**Round 3 — Abdu's review (approve + 3 amendments + 2 findings, all folded in):** **Fork 2** the GL-origin space is now a **mandated type**, not a deferred smell — glow-facing surfaces accept only it, constructed never flipped, any literal `fb_height − y` in the diff a red flag (§5). **Fork 1** kept one feature-gated crate but added the **`cargo tree -i wgpu` feature-unification guard** + a publish-trigger to fold the split into the M5 break (§7/§17). **Unsafe signed off** with two conditions: **no GL in `Drop`** + an explicit `destroy(&glow::Context)` from `on_exit` closing the native teardown UB hole (§6/§11), and **`#![deny(clippy::undocumented_unsafe_blocks)]`** to mechanize the SAFETY discipline (§11). **Finding 1:** the ran-flag protocol was *impossible* as written (`prepare` runs inside the callback, so `frost` can't gate on its result) — corrected to flag-at-callback-entry (§7). **Finding 2:** the live target is captured via `GL_DRAW_FRAMEBUFFER_BINDING` at callback entry so grab+composite+encode all follow egui's real target — which **dissolves the `pp_fb_extent` caveat** into handled behavior (§5/§10).
