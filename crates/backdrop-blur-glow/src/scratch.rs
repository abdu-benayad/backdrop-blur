...
use backdrop_blur_core::{BlurError, BlurStage, PingPongKey, kawase_level_size};
use glow::HasContext;
use std::collections::HashMap;

/// How many frames a scratch chain may go untouched before eviction. Small: a panel that stops
/// being frosted should not pin its VRAM for long, but a chain must survive a few frames of churn
/// (a drag resizes every frame) without thrashing.
const RETENTION_FRAMES: u64 = 8;
...
/// The eviction decision — **pure**, so it is Tier-0 testable with no GL. Given each chain's key
/// and the frame it was last used, the current frame, and the retention window, return the keys
/// whose `now - last_used >= retention` (using wrapping-aware distance so a `u64` frame counter
/// that wraps does not spuriously evict). A chain touched *this* frame (`distance 0`) is never
/// evicted.
pub(crate) fn evict_decision(
    chains: impl IntoIterator<Item = (PingPongKey, u64)>,
    now: u64,
    retention: u64,
) -> Vec<PingPongKey> {
    chains
